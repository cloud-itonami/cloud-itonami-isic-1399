(ns nonwovenops.store
  "SSoT for the nonwoven/felt technical-textiles plant operations
  ...
  external nonwoven/technical-textiles capability library, no
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             forming/bonding-line output: quality-
                             grade/area/tensile-strength/basis-weight/
                             defect-rate record (nonwoven fabric or
                             felt, including technical/industrial
                             textiles such as tire-cord fabric).
                             `:verified?` marks whether the batch's
                             own claims have actually been
                             QC-inspected -- specifically, whether the
                             batch's own tensile-strength-test and
                             basis-weight-test results have actually
                             been measured (never inferred from a
                             routine intake patch); `:registered?`
                             marks whether it is on file in the
                             plant's production ledger;
                             `:shipped-area-square-meters` tracks the
                             batch's own cumulative-shipped ground
                             truth.
    - `equipment`         -- a forming-line/bonding-line unit's own
                             ...
                             `batches`.
    - `maintenance`       -- a scheduled forming-line or bonding-line
                             maintenance-window DRAFT against a piece
                             of equipment (`nonwovenops.registry`'s
                             `register-maintenance`). Dedicated
                             `:scheduled?` double-schedule guard (never
                             a `:status` value).
    - `shipments`         -- a proposed outbound nonwoven/felt
                             shipment DRAFT (`nonwovenops.registry`'s
                             `register-shipment`).
  ...
  audit trail a plant owner or nonwoven/felt buyer trusting this
  coordinator needs."
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (nonwovenops.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (nonwovenops.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :quality-grade :grade-a :product "spunbond-polypropylene-nonwoven"
                :area-square-meters 20000.0 :tensile-strength-kn 12.0 :basis-weight-gsm 150.0
                :defect-rate-percent 1.2
                :verified? true :registered? true
                :shipped-area-square-meters 5000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :quality-grade :grade-b :product "needle-punched-polyester-felt"
                :area-square-meters 6000.0 :tensile-strength-kn 18.0 :basis-weight-gsm 400.0
                :defect-rate-percent 3.0
                :verified? true :registered? true
                :shipped-area-square-meters 5700.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :quality-grade :substandard :product "meltblown-filtration-media"
                :area-square-meters 15000.0 :tensile-strength-kn 2.5 :basis-weight-gsm 25.0
                :defect-rate-percent 18.0
                :verified? false :registered? false
                :shipped-area-square-meters 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :forming-line
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :bonding-line
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `nonwovenops.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `nonwovenops.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defrecord MemStore [a]
  ...
  (commit-record! [s {:keys [effect path value] :as record}]
    (cond
      ...
      (let [shipment-id (first path)
        ...
        (swap! a (fn [state]
                   (-> state
                       ...
                       (update-in [:batches batch-id :shipped-area-square-meters]
                                  (fn [prev]
                                    (+ (double (or prev 0.0))
                                       (double (or (:area-square-meters value) 0.0))))))))
        result)
      ...
      ;; above (also what `cordageops`-style siblings expose as their
...
(defn sample-data!
  ...
  shipped (a small new shipment blows through its own logged area --
  HARD hold), one UNVERIFIED/unregistered batch (blocks any shipment
  coordinated against it); one verified+registered forming-line unit
  (schedulable for maintenance), one UNVERIFIED/unregistered bonding-
  line unit (blocks any maintenance scheduling against it) -- so the
