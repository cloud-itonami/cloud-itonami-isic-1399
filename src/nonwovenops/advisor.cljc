(ns nonwovenops.advisor
  "NonwovenAdvisor -- the *contained intelligence node* for the
  nonwoven/felt technical-textiles-plant plant-operations coordination
  actor.

  It normalizes production-batch patches (quality-grade/area/tensile-
  strength/basis-weight/defect-rate), drafts a forming-line or
  bonding-line maintenance scheduling proposal against a piece of
  equipment, drafts an equipment-safety/quality-defect concern flag,
  and drafts an outbound-nonwoven/felt-shipment coordination proposal
  ...
  cited), never a committed record and NEVER a real forming/bonding-
  line-equipment actuation or freight dispatch. Every output is
  censored downstream by `nonwovenops.governor` before anything
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `nonwovenops.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; bonding-line-equipment-control
                                 ; effect
  ...
  `nonwovenops.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [nonwovenops.registry :as registry]
...
(defn- log-production-batch
  ...
  area, tensile-strength, basis-weight or defect-rate. High confidence,
...
(defn- schedule-maintenance
  "Draft a forming-line or bonding-line maintenance-window scheduling
  ...
  `nonwovenops.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  ...
  `nonwovenops.phase`: no phase ever adds this op to a phase's `:auto`
  set; `nonwovenops.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound nonwoven/felt shipment coordination proposal
  against a production batch. The advisor passes through the caller's
  own claimed area -- it does NOT invent one, and
  `nonwovenops.governor` NEVER trusts it: it independently recomputes
  whether the batch's own cumulative-shipped area plus this claim
  would exceed the batch's own recorded area before any commit is
  possible."
  [db {:keys [subject value]}]
  (let [batch-id (:batch-id value)
        ...
        over-area? (and b (registry/shipment-area-exceeded?
                           b (:area-square-meters value)))]
    {:summary    (str subject " 向け出荷調整提案 ("
                      (:area-square-meters value) " 平方メートル)"
...
(def ^:private system-prompt
  (str "あなたは不織布・フェルト等技術繊維工場プラント運用コーディネーターの助言者です。"
       ...
       "成形ライン・接合ラインの設備の直接操作を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `nonwovenops.governor`
