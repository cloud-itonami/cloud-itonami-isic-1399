(ns nonwovenops.operation
  "NonwovenOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (NonwovenAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Nonwoven & Technical
  Textiles Plant Operations Governor (:govern) and the rollout phase
  ...
  (:require [langgraph.graph :as g]
            ...
            [nonwovenops.advisor :as advisor]
            [nonwovenops.governor :as governor]
            [nonwovenops.phase :as phase]
...
(defn build
  "Compiles a NonwovenOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `nonwovenops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; NonwovenAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Nonwoven & Technical Textiles Plant Operations Governor -- independent censor.
      (g/add-node :govern
