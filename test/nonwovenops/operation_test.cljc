(ns nonwovenops.operation-test
  "Smoke tests for the compiled NonwovenOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `nonwovenops.governor-contract-test`; the Store contract in
  `nonwovenops.store-contract-test`."
...
(deftest test-actor-builds
  (testing "NonwovenOperationActor can be built with a store"
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "equip-001" :concern-type :equipment-safety
                                   :severity :moderate :description "matting near bonding line"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
