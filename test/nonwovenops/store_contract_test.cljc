(ns nonwovenops.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `nonwovenops.store` ns docstring for why a second (Datomic-
...
(deftest shipment-propose-commits-and-advances-sequence-and-batch-area
  (let [s (seeded)]
    ...
    (is (= 10000.0 (:shipped-area-square-meters (store/batch s "batch-001")))
