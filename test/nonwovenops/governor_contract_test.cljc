(ns nonwovenops.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control forming or bonding line equipment
  directly... does NOT authorize or execute equipment operations')
  implemented faithfully. The single invariant under test:

    NonwovenAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Nonwoven & Technical
    Textiles Plant Operations Governor would reject; `:schedule-
...
(deftest invalid-tensile-strength-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:tensile-strength-kn 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-tensile-strength} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:tensile-strength-kn (store/batch db "batch-001"))) "fabricated tensile-strength never lands in the SSoT")))

(deftest invalid-basis-weight-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:basis-weight-gsm 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-basis-weight} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:basis-weight-gsm (store/batch db "batch-001"))) "fabricated basis-weight never lands in the SSoT")))
